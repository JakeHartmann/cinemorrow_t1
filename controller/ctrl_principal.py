...
class CtrlPrincipal():
    ...
    def tipo_de_midia_mais_favorita(self):
        midias_fav = [pessoa.midia_fav for pessoa in self.ctrl_pessoa.pessoas if pessoa.midia_fav]
        ...
        if midias_fav:
            ...
            if len(mais_comuns) > 1 and mais_comuns[0][1] == mais_comuns[1][1]:
                ...
                print(f"O tipo de midia favorita mais comum é: {tipo_midia_mais_comum}, Quantidade: {quantidade}")
    ...
    def pessoa_em_mais_grupos(self):
        if self.ctrl_grupo.grupos:
            contador_pessoa = Counter([pessoa for grupo in self.ctrl_grupo.grupos for pessoa in grupo.pessoas])
            if contador_pessoa:
                ...
                if len(pessoas_empate) == 1:
                    ...
                    print(f"A pessoa no maior número de grupos é: {pessoa_mais_comum.nome}, Quantidade de grupos: {quantidade_max}")
                else:
                    print("Empate! Pessoas com o mesmo número de ocorrência em grupos: ")
    ...
    def grupo_com_mais_pessoas(self):
        if self.ctrl_grupo.grupos:
            ...
            if grupos_por_quantidade:
                ...
                grupos_tie = [grupo for grupo in self.ctrl_grupo.grupos if len(grupo.pessoas) == max_quantidade]

                if len(grupos_tie) == 1:
                    grupo_mais_pessoas = grupos_tie[0]
                    quantidade_pessoas = len(grupo_mais_pessoas.pessoas)
                    print(f"O grupo com mais pessoas é: {grupo_mais_pessoas.nome}, Quantidade: {quantidade_pessoas} pessoa(s)")
                else:
                    print(f"Empate! Grupos com o mesmo número de pessoas ({max_quantidade}):")
    ...
    def tipo_de_midia_mais_assistida(self):
        midias_associadas = [grupo.midia_associada for grupo in self.ctrl_grupo.grupos if grupo.midia_associada]
        
        if midias_associadas:
            tipo_midia_mais_vista, quantidade = Counter(midias_associadas).most_common(1)[0]
            print(f"A mídia mais assistida é: {type(tipo_midia_mais_vista).__name__}, Quantidade: {quantidade}")
        else:
            print("Sem informações disponíveis.")
    ...
    def tipo_de_midia_mais_assistida(self):
        ...
        if midias_associadas:
            ...
            empate_midias = [midia for midia, quantidade in midias_mais_vistas[1:] if quantidade == quantidade]

            if len(empate_midias) == 0:
                print(f"O tipo de mídia mais visto é: {tipo_midia_mais_vista.titulo}, Quantidade: {quantidade}")
            else:
                print(f"Empate! Tipos de mídia com a mesma quantidade de visualizações ({quantidade}):")
    ...
    def abre_tela(self):
        ...
        while True:
            opcao = self.tela_principal.recebe_input_int("Escolha uma opção (principal): ",[1, 2, 3, 4, 5])
            escolha = lista_opcoes[opcao]
            escolha()
